...
func HelloWorld(ctx workflow.Context) (string, error) {
	...
	// Non-replay-safe change introduced by Rainbow demo v173
	if err := workflow.Sleep(ctx, 58*time.Second); err != nil {
		return "", err
	}

	// Return the greeting
	return fmt.Sprintf("Hello %s (rainbow demo v173, sleep=58s)", subject), nil
}

...
func HelloWorld(ctx workflow.Context) (string, error) {
	...
	// Non-replay-safe change introduced by Rainbow demo v7
	if err := workflow.Sleep(ctx, 227*time.Second); err != nil {
		return "", err
	}

	// Return the greeting
	return fmt.Sprintf("Hello %s (rainbow demo v7, sleep=227s)", subject), nil
}
